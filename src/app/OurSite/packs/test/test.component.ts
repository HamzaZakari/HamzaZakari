...
added = false;
  ...
  proID;
  subs4: Subscription;
  constructor(private pack: PackserviceService,private serv: ServicesService,  private services: ServicesService,@Inject(DOCUMENT) private document: Document) { }
  ...
  ngOnInit(): void {
    ...
    this.pack.getcat_meil().subscribe(data => {
  ...
  getproPoids(id) {
    this.serv.gettingpoids(id);
    this.subs4 = this.serv.emitPoidspro().subscribe(data => {
  ...
  addtocart(product_id,poids?){
    //...
    this.serv.addtoocart(product_id, poids);
    this.added = true;
    this.serv.gettotaleitem();
    this.serv.getitemincart();
